...
const model = process.env.PUBLIC_GPT_MODEL || process.env.GPT_MODEL || MODEL;
...
`;
  ...
  try {
    ...
    console.log("API 키 상태:", apiKey ? "설정됨 (마지막 4자리: " + apiKey.slice(-4) + ")" : "설정되지 않음"); 
    ...
    // 오류의 원인에 따라 다른 메시지 출력
    if (error.response) {
      console.error("API 응답 오류:", error.response.status, error.response.data);
    } else if (error.request) {
      console.error("API 요청은 보냈으나 응답을 받지 못함");
    } else {
      console.error("API 요청 생성 중 오류 발생");
    }
...
// 키워드 목록에서 주제 가져오기
function getTopicFromKeywords() {
  try {
    const keywordsPath = path.join(__dirname, 'keywords.json');
    const keywords = JSON.parse(fs.readFileSync(keywordsPath, 'utf8'));
    
    if (!Array.isArray(keywords) || keywords.length === 0) {
      console.log('키워드 목록이 비어 있습니다.');
      return null;
    }
    
    // 임의의 키워드 선택 (또는 순차적으로 선택하도록 구현 가능)
    return keywords[Math.floor(Math.random() * keywords.length)];
  } catch (error) {
    console.error('키워드 파일을 읽는 중 오류가 발생했습니다:', error);
    return null;
  }
}
...
async function main() {
  try {
    // 명령행 인수에서 주제 가져오기 또는 키워드 목록에서 가져오기
    let topic = process.argv[2];
    
    if (!topic) {
      topic = getTopicFromKeywords();
      if (!topic) {
        console.log('사용법: node generatePost.js "블로그 글 주제"');
        process.exit(1);
      }
    }
    ...
    // 파일 저장 전 약간의 지연
    console.log("3. 파일 저장 준비 중...");
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    // 생성된 글 저장
    console.log("4. 파일 저장 시작...");
    const saved = await saveBlogPost(blogContent, topic);
    
    if (saved) {
      console.log("5. 프로세스 완료: 블로그 글이 성공적으로 생성되고 저장되었습니다!");
    } else {
      console.log("5. 프로세스 완료: 블로그 글이 생성되었지만 저장되지 않았습니다.");
    }
