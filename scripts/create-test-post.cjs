...
// 포스트 파일명
const postFileName = `${formattedDate}-healthy-eating-for-middle-aged-women.md`;
const postFilePath = path.join(postsDir, postFileName);

// 이미지 경로 설정 (이미지는 이미 존재한다고 가정)
const imagePath = '/images/posts/healthy-eating-cover.jpg';
...
console.log(`이미지 경로가 설정되었습니다: ${imagePath}`);

// 이미지 경로 안내 메시지
console.log('참고: 실제 이미지 파일은 public/images/posts 디렉토리에 직접 추가해야 합니다.'); 